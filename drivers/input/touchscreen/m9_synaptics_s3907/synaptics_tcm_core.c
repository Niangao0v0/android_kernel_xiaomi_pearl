...
static bool flag_selftest = true;
...
int syna_tcm_add_module(struct syna_tcm_module_cb *mod_cb, bool insert)
{
	...
	while (!mod_pool.initialized) {
		if (mod_pool_wait_cnt > 0) {
			pr_err("%s: mod_pool.initialized is not ready, and waint cnt(%d) > 0, give up this module(type:%d, instsert:%s)\n",
				__func__, mod_pool_wait_cnt, mod_cb->type, (insert) ? "true" : "false");
			return -EINVAL;
		}
	...
	if (waitTimeMs > 0) {
		...
		pr_err("%s: wait time %d ms for module(type:%d, instsert:%s)\n",
			__func__, waitTimeMs, mod_cb->type, (insert) ? "true" : "false");
	}
...
static int syna_tcm_config_gpio(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (bdata->reset_gpio >= 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "%s set reset_active_ms:%d reset_delay_ms:%d \n", __func__, bdata->reset_active_ms, bdata->reset_delay_ms);
...
static int syna_tcm_sleep(struct syna_tcm_hcd *tcm_hcd, bool en)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				...
				STR(CMD_EXIT_DEEP_SLEEP), retval);
		goto exit;
	}
...
static void syna_tcm_helper_work(struct work_struct *work)
{
	...
	case HELP_TOUCH_REINIT:
		...
		syna_tcm_set_cur_value(Touch_Fod_Enable, FOD_STATUS_AUTHENTICATION);
#endif
...
static int syna_tcm_pm_resume(struct device *dev)
{
	...
	if (retval < 0) {
		...
		LOGE(tcm_hcd->pdev->dev.parent,"Failed to exit sleep resulting in frozen screen \n");
		goto exit;
	}else{
		LOGN(tcm_hcd->pdev->dev.parent," one exit deep sleep success retval=%d\n",retval);
	}
...
static int syna_tcm_early_suspend(struct device *dev)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "fod_enabled:%d aod_enable:%d doubletap_enable:%d finger_unlock_status:%d\n", 
		tcm_hcd->fod_enabled, tcm_hcd->aod_enable, tcm_hcd->doubletap_enable, tcm_hcd->finger_unlock_status);
	...
	if (tcm_hcd->wakeup_gesture_enabled && tcm_hcd->finger_unlock_status > 0) {
		LOGI(tcm_hcd->pdev->dev.parent, "finger_unlock_status[%d] set fod_enabled to enable\n", tcm_hcd->finger_unlock_status);
	...
	if (!tcm_hcd->wakeup_gesture_enabled || tcm_hcd->nonui_status == 2) {
		...
		LOGN(tcm_hcd->pdev->dev.parent, "skip set fw sleep for wakeup_gesture_enabled:%d nonui_status:%d \n", tcm_hcd->wakeup_gesture_enabled, tcm_hcd->nonui_status);
...
static void syna_tcm_early_suspend_work(struct work_struct *work)
{
	...
	if (!tcm_hcd->in_sleep)
		...
		LOGN(tcm_hcd->pdev->dev.parent, "tp fw is in sleep, skip palm area change setting\n");
...
static int syna_tcm_drm_state_notifier_callback(struct notifier_block *nb, unsigned long action, void *data)
{
	int retval = 0;
	int transition;
	...
	if (!(action == MI_DISP_DPMS_EARLY_EVENT || action == MI_DISP_DPMS_EVENT || action == MI_DISP_CHANGE_FPS)) {
		LOGE(tcm_hcd->pdev->dev.parent, "event(%lu) do not need process\n", action);
		return 0;
	}
	...
	if (evdata && evdata->data && tcm_hcd) {
		...
		if (action == MI_DISP_DPMS_EARLY_EVENT && (transition == MI_DISP_DPMS_POWERDOWN)) {
			LOGN(tcm_hcd->pdev->dev.parent, "touch early_suspend by powerdown 0x%04x\n", transition);
			queue_work(tcm_hcd->event_wq, &tcm_hcd->early_suspend_work);
				...
				} else if (transition == MI_DISP_DPMS_LP1 || transition == MI_DISP_DPMS_LP2){
					LOGN(tcm_hcd->pdev->dev.parent,
			...
			LOGI(tcm_hcd->pdev->dev.parent, "tp received fps change, new:%d\n", transition);
	}
...
static void syna_tcm_set_report_rate_work(struct work_struct *work)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"set report rate failed, retval=%d\n", retval);
	}
...
static void syna_tcm_fod_work(struct work_struct *work)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"set touch and hold enable control failed, retval=%d\n", retval);
	}
...
static void syna_tcm_corner_rejection(bool on, int direction, struct syna_grip_zone *zone)
{
	...
	LOGI(tcm_hcd->pdev->dev.parent,  "filter_value = %d\n", filter_value);
...
#ifdef GRIP_MODE_DEBUG
		pVal = (int *)&(bdata->cornerzone_filter_hor1[0]);
		LOGI(tcm_hcd->pdev->dev.parent, "direction == 1 cornerzone_hor1 %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
				pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6], pVal[7], pVal[8], pVal[9], pVal[10], pVal[11],
				pVal[12], pVal[13], pVal[14], pVal[15],	pVal[16], pVal[17], pVal[18], pVal[19], pVal[20], pVal[21], pVal[22], pVal[23],
				pVal[24], pVal[25], pVal[26], pVal[27], pVal[28], pVal[29], pVal[30], pVal[31]);
#endif
...
#ifdef GRIP_MODE_DEBUG
		pVal = (int *)&(bdata->cornerzone_filter_hor2[0]);
		LOGI(tcm_hcd->pdev->dev.parent, "direction == 3 cornerzone_hor2 %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
				pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6], pVal[7], pVal[8], pVal[9], pVal[10], pVal[11],
				pVal[12], pVal[13], pVal[14], pVal[15],	pVal[16], pVal[17], pVal[18], pVal[19], pVal[20], pVal[21], pVal[22], pVal[23],
				pVal[24], pVal[25], pVal[26], pVal[27], pVal[28], pVal[29], pVal[30], pVal[31]);
#endif
...
#ifdef GRIP_MODE_DEBUG
		pVal = (int *)&(bdata->cornerzone_filter_ver[0]);
		LOGI(tcm_hcd->pdev->dev.parent, "direction == 0 cornerzone_ver %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d %2d\n",
				pVal[0], pVal[1], pVal[2], pVal[3], pVal[4], pVal[5], pVal[6], pVal[7], pVal[8], pVal[9], pVal[10], pVal[11],
				pVal[12], pVal[13], pVal[14], pVal[15],	pVal[16], pVal[17], pVal[18], pVal[19], pVal[20], pVal[21], pVal[22], pVal[23],
				pVal[24], pVal[25], pVal[26], pVal[27], pVal[28], pVal[29], pVal[30], pVal[31]);
#endif
...
static void syna_tcm_update_grip_mode(void)
{
	...
	*/
	...
	if (!gamemode_on) {
		LOGI(tcm_hcd->pdev->dev.parent, "non gamemode\n");
...
static void syna_tcm_get_grip_mode(unsigned int *gamemode, unsigned int *panel_orientation)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to write command %s to get grip setting, retval=%d\n", STR(CMD_MultiFunction), retval);
		goto exit;
	}
	LOGI(tcm_hcd->pdev->dev.parent,"write_message success");
	...
	*panel_orientation = resp_buf[0] & 0x0F;
	...
	offset = 0;
	LOGI(tcm_hcd->pdev->dev.parent, "length:%d, gamemode|panel_orientation: %d %d, checksum: %d\n",
...
static void syna_tcm_update_touchmode_data(void)
{
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		if (p_xiaomi_touch_interfaces->touch_mode[i][GET_CUR_VALUE] != p_xiaomi_touch_interfaces->touch_mode[i][SET_CUR_VALUE]) {
			...
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent, "set dynamic config id:0x%02x failed, retval=%d\n", dc_id, retval);
			}

			// debug: read to get the current value in FW
			retval = tcm_hcd->get_dynamic_config(tcm_hcd, dc_id, &temp_value);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,	"Failed to get dynamic config id: 0x%02x, retval=%d\n", dc_id, retval);
			} else {
...
static void syna_tcm_grip_mode_work(struct work_struct *work)
{
	...
	int long_mode_len;
	LOGI(tcm_hcd->pdev->dev.parent,	"%s enter\n", __func__);
	mutex_lock(&tcm_hcd->long_mode_value_mutex);
...
static int syna_tcm_read_touchmode_data(void)
{
	...
	LOGI(tcm_hcd->pdev->dev.parent,
		"game_mode:%d, active_mode:%d, up_threshold:%d, tolerance:%d, panel orientation:%d, Report Rate:%d\n",
		get_value[Touch_Game_Mode], get_value[Touch_Active_MODE], get_value[Touch_UP_THRESHOLD],
...
static int syna_tcm_palm_area_change_setting(int value)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"Failed to palm area change setting, retval=%d\n", retval);
		goto exit;
	}
...
static int syna_tcm_set_cur_value(int mode, int val)
{
	...
	case Touch_Fod_Enable:
		...
		if (val == FOD_STATUS_INVALID || val == FOD_STATUS_DELETED) {
			tcm_hcd->fod_enabled = FLAG_FOD_DISABLE;
		}
		else if (val == FOD_STATUS_DISABLED || val == FOD_STATUS_INPUT_FINGERPRINT) {
			tcm_hcd->finger_unlock_status = val;
			tcm_hcd->fod_enabled = FLAG_FOD_ENABLE;
		} else if (val == FOD_STATUS_IN_SUSPENDING) {
			tcm_hcd->finger_unlock_status = val;
			tcm_hcd->fod_enabled = FLAG_FOD_DISABLE;
		}else if (val == FOD_STATUS_AUTHENTICATION) {
			tcm_hcd->finger_unlock_status = val;
			tcm_hcd->fod_enabled = FLAG_FOD_ENABLE;
			tcm_hcd->wakeup_gesture_enabled = WAKEUP_GESTURE;
			if (tcm_hcd->in_sleep && tcm_hcd->in_suspend && tcm_hcd->flag_sleep) {
				LOGI(tcm_hcd->pdev->dev.parent, "Touch_Fod_Enable value:FOD_STATUS_AUTHENTICATION, Exit sleep mode!\n");
				/* enable irq */
				tcm_hcd->enable_irq(tcm_hcd, true, NULL);
				retval = tcm_hcd->sleep(tcm_hcd, false);
				if (retval < 0) {
		...
		LOGI(tcm_hcd->pdev->dev.parent, "Touch_Fod_Enable set fod_enabled to %d , set finger_unlock_status to %d\n", tcm_hcd->fod_enabled, tcm_hcd->finger_unlock_status);
		queue_work(tcm_hcd->event_wq, &tcm_hcd->fod_work);
		break;
...
static int syna_tcm_parse_gamemode_param_dt(struct syna_tcm_xiaomi_board_data *bdata)
{
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		...
		if (prop && prop->length) {
			...
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent, "Failed to read synaptics, %s property, retval=%d\n", str_dts, retval);
			}
		}
		...
		LOGI(tcm_hcd->pdev->dev.parent, "%s: %2d %2d %2d %2d %2d\n",
...
static int syna_tcm_get_charging_status()
{
	...
	if (usb_psy) {
		...
		if (rc < 0)
			LOGE(gloab_tcm_hcd->pdev->dev.parent, "%s Couldn't get usb online status, rc=%d\n", __func__, rc);
...
static void syna_tcm_power_supply_work(struct work_struct *work)
{
	...
	if (charging_status != gloab_tcm_hcd->charging_status || gloab_tcm_hcd->charging_status < 0) {
		...
		gloab_tcm_hcd->charger_connected = (charging_status == WIRED_CHARGING) ? 1 : 0;
				 ...
				 size_t count, loff_t *pos)
{
	...
	if (*pos == 0) {
		...
		//cnt1 = testing_xiaomi_report_data(REPORT_DELTA, tmp); /* REPORT_DELTA = 0x12 */
		LOGD(tcm_hcd->pdev->dev.parent, "cnt1 = %d\n", cnt1);
		if (cnt1 <= 0) {
			LOGE(tcm_hcd->pdev->dev.parent, "REPORT_DELTA failed, cnt1=%d\n", cnt1);
			ret = cnt1;
			goto err_out;
		}

		cnt2 = tcm_hcd->testing_xiaomi_report_data(REPORT_RID161, tmp + cnt1); /* REPORT_RID161 = 0xA1 */
		//cnt2 = testing_xiaomi_report_data(REPORT_RID161, tmp + cnt1); /* REPORT_RID161 = 0xA1 */
		LOGD(tcm_hcd->pdev->dev.parent, "cnt2 = %d\n", cnt2);
		if (cnt2 <= 0) {
			LOGE(tcm_hcd->pdev->dev.parent, "REPORT_RID161 failed, cnt2=%d\n", cnt2);
			ret = cnt2;
			goto err_out;
		}
		sum_size = cnt1 + cnt2;
...
static void syna_tcm_selftest_work(struct work_struct *work)
{
	struct syna_tcm_hcd *tcm_hcd = gloab_tcm_hcd;
	int retval = 0;
	char *self_test_data = NULL;

	if (!tcm_hcd->testing_xiaomi_self_test) {
		LOGE(tcm_hcd->pdev->dev.parent, "%s NULL Pointer!\n", __func__);
		retval = 1;
		goto out;
	}
	...
	tcm_hcd->selftest_ready = true;
	wake_up_interruptible(&tcm_hcd->selftest_wait_queue);
	return;
				  ...
				  size_t count, loff_t *pos)
{
	...
	if (!tcm_hcd)
		return 0;
	if ((!buf) || (count > 6)) {
		LOGE(tcm_hcd->pdev->dev.parent, "%s , count = %d!\n", __func__, count);
		retval = 1;
		goto out;
	}
	...
	if (!strncmp("short", tmp, 5) || !strncmp("open", tmp, 4)) {
		tcm_hcd->selftest_ready = false;
		queue_work(tcm_hcd->selftest_wq, &tcm_hcd->selftest_work);
		wait_event_interruptible(tcm_hcd->selftest_wait_queue, tcm_hcd->selftest_ready);
		if(flag_selftest == false){
			retval = 1;
			goto out;
		}
		retval = 2;
...
static u8 syna_tcm_panel_vendor_read(void)
{
	...
	if (tp_probe_success && tcm_hcd) {
		if (!tcm_hcd->syna_tcm_lockdown_info) {
			LOGE(tcm_hcd->pdev->dev.parent,
				"%s: get lockdown info error, syna_tcm_lockdown_info==NULL\n", __func__);
			return 0;
		}
		ret = tcm_hcd->syna_tcm_lockdown_info();
		if (ret) {
...
*/
static int syna_pinctrl_init(struct syna_tcm_hcd *tcm_hcd, struct spi_device *spi){
	int retval = 0;
	/* Get pinctrl if target uses pinctrl */
	tcm_hcd->ts_pinctrl = devm_pinctrl_get(&spi->dev);
	if (IS_ERR_OR_NULL(tcm_hcd->ts_pinctrl)) {
		retval = PTR_ERR(tcm_hcd->ts_pinctrl);
		LOGE(tcm_hcd->pdev->dev.parent, "Target does not use pinctrll=%d\n", retval);
	...
	if (IS_ERR_OR_NULL(tcm_hcd->pinctrl_state_spi)) {
		...
		LOGE(tcm_hcd->pdev->dev.parent, "Can not lookup %s pinstate %d\n", PINCTRL_STATE_SPI,retval);
...
*/
	...
	if (!retval && tcm_hcd->ts_pinctrl){
		retval = pinctrl_select_state(tcm_hcd->ts_pinctrl, tcm_hcd->pinctrl_state_spi);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
	...
	tcm_hcd->selftest_wq = create_singlethread_workqueue("selftest_wq");
	if (!tcm_hcd->selftest_wq) {
		retval = -ENOMEM;
	}
	INIT_WORK(&tcm_hcd->selftest_work, syna_tcm_selftest_work);
	init_waitqueue_head(&tcm_hcd->selftest_wait_queue);
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "ERROR: register notifier failed, retval=%d\n", retval);
	}
...
err_enable_irq:
	...
	flush_workqueue(tcm_hcd->selftest_wq);
	destroy_workqueue(tcm_hcd->selftest_wq);
...
err_enable_regulator:
	if (!tcm_hcd->avdd || !tcm_hcd->iovdd)
		LOGE(tcm_hcd->pdev->dev.parent, "There is NULL for avdd or iovdd\n");
