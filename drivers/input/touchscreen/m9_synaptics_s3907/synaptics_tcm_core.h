...
#define LOGD(dev, log, ...) \
	LOGx(dev_dbg, dev, log, ##__VA_ARGS__)
#define LOGI(dev, log, ...) \
	LOGx(dev_info, dev, log, ##__VA_ARGS__)
#define LOGN(dev, log, ...) \
	LOGx(dev_notice, dev, log, ##__VA_ARGS__)
#define LOGW(dev, log, ...)	\
	LOGy(dev_warn, dev, log, ##__VA_ARGS__)
#define LOGE(dev, log, ...) \
	LOGy(dev_err, dev, log, ##__VA_ARGS__)
...
enum fod_status {
	FOD_STATUS_INVALID = -1,	/*as default value, means fingerprint is not enabled*/
	FOD_STATUS_DISABLED = 0,	/*means fingerpirint is not enabled*/
	FOD_STATUS_AUTHENTICATION = 1,	/* means fingerprint is in authentication*/
	FOD_STATUS_INPUT_FINGERPRINT = 2,	/*means fingerprint is in enroll*/
	FOD_STATUS_IN_SUSPENDING = 3,	/*means fingerprint is to be set when suspend*/
